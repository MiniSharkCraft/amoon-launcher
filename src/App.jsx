...
// ─── Login Modal (TLauncher style) ───────────────────────────────────────────
function LoginModal({ onClose }) {
  const { loginOffline, loginMicrosoftStart, loginElybyStart, accountLoading, accountError, accounts, activeAccountId, switchAccount, removeAccount } = useStore();
  const [authType, setAuthType] = useState("offline");
  const [username, setUsername] = useState("");
  const [password, setPassword] = useState("");
  const prevLen = useRef(accounts.length);

  useEffect(() => { prevLen.current = accounts.length; }, [accounts.length]);
  ...
  const handleAdd = async () => {
    if (authType === "offline") {
      ...
      try { await loginMicrosoftStart(); } catch { /* error in store */ }
    } else if (authType === "elyby") {
      try { await loginElybyStart(); } catch { /* error in store */ }
    }
    // amoon: not yet implemented
  };
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100 }}>
      <div style={{ background:"#1a2035", border:`1px solid ${C.border}`, borderRadius:14, width:580, overflow:"hidden" }}>
        ...
        <div style={{ background:C.accent, padding:"12px 20px", display:"flex", alignItems:"center", gap:12 }}>
          ...
          <span style={{ fontSize:14, fontWeight:600, color:"white", display:"inline-flex", alignItems:"center", gap:6 }}><ShieldCheck size={16} weight="bold"/> Account settings</span>
        </div>
        ...
        <div style={{ display:"flex", gap:0 }}>
          ...
          <div style={{ flex:1, padding:"20px", borderRight:`1px solid ${C.border}` }}>
            ...
            {/* Password (chỉ hiện khi không phải offline) */}
            {authType !== "offline" && (
              <input
            ...
            <button
              onClick={handleAdd}
              disabled={(authType==="offline" && !username.trim()) || authType==="amoon" || accountLoading}
              style={{ width:"100%", padding:"10px 0", borderRadius:8, border:"none", background:authType==="amoon"?"#6b7280":"#22c55e", color:"white", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit", opacity:((authType==="offline"&&!username.trim())||authType==="amoon"||accountLoading)?0.5:1 }}
            >
          ...
          <div style={{ width:200, padding:"20px 16px" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:12 }}>
              ...
              {accounts.map(acc=>(
                ...
                >
                  <div style={{ width:24, height:24, borderRadius:6, background:`linear-gradient(135deg,${C.accent},#1d4ed8)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:"white", flexShrink:0 }}>
            ...
            <div style={{ display:"flex", gap:6 }}>
              <button
                ...
                style={{ flex:1, padding:"8px 0", borderRadius:7, border:"none", background:C.accent, color:"white", fontSize:12, fontWeight:500, cursor:"pointer", fontFamily:"inherit", opacity:(!username.trim()||accountLoading)?0.5:1 }}
              >
                Add
              </button>
              <button
                ...
                style={{ flex:1, padding:"8px 0", borderRadius:7, border:"none", background:"#f97316", color:"white", fontSize:12, fontWeight:500, cursor:"pointer", fontFamily:"inherit", opacity:!activeAccountId?0.5:1 }}
              >
                Remove
              </button>
            </div>
